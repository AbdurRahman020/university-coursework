...
def is_move_ok(grid, x_to, y_to):
    ...
    # method#3
    if (0 <= x_to < grid.width) and (0 <= y_to < grid.height):
        return True
    else:
       return False
    '''
...
def move_water(grid, x_from, y_from):
    ...
    # down
    if is_move_ok(grid, x_from, y_from + 1) and grid.get(x_from, y_from + 1
                                                         ) is None:
    ...
    '''
    # randomly choose between down left or down right
    possible_moves = []
    
    # down right
    if is_move_ok(grid, x_from + 1, y_from + 1) and grid.get(x_from + 1, y_from + 1) is None:
    ...
    # if both down left and down right are valid, randomly pick one
    if possible_moves:
        x_offset, y_offset = random.choice(possible_moves)
        grid.set(x_from, y_from, None)
        grid.set(x_from + x_offset, y_from + y_offset, 'w')
        return grid
    '''

    # down right
    if is_move_ok(grid, x_from + 1, y_from + 1) and grid.get(
            x_from + 1, y_from + 1) is None:
        grid.set(x_from, y_from, None)
    ...
    # if no valid move, erase the water
    grid.set(x_from, y_from, None)
...
def draw_grid_canvas(grid, canvas):
    ...
    """
    canvas.delete('all')
    canvas.create_rectangle(0, 0, grid.width * SIDE,
                            grid.height * SIDE, fill='black')
    ...
    for y in range(grid.height):
        for x in range(grid.width):
            ...
            if val:
                ...
                pixel_y = SIDE * y
                canvas.create_text(
                    pixel_x, pixel_y, text=val, anchor=tkinter.NW, fill='white', font=('Courier', 20))
