...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    if abs(x_from - x_to) == 1 and abs(y_from - y_to) == 1:
        if grid.in_bounds(x_to, y_to - 1) and grid.get(x_to, y_to - 1
                                                       ) is not None:
            return False
...
def do_gravity(grid, x, y):
    ...
    move down, move down-left, move down-right, move left, move right.
...
def do_brownian(grid, x, y, brownian):
    """
    Given grid, x, y, and brownian strength (0..100),
    performs a random Brownian motion for the sand at position (x, y).
    Return the grid.

    >>> random.randrange = lambda n: 0  # For doctests
    >>> grid = Grid.build([[None, 's', None], ['s', 's', None]])
    ...
    if num < brownian:
        ...
        # only move if within bounds and target position is empty
        if grid.in_bounds(x_new, y_new) and grid.get(x_new, y_new) is None:
            do_move(grid, x, y, x_new, y_new)
...
def do_whole_grid(grid, brownian):
    ...
    for y in reversed(range(grid.height)):
        ...
        for x in range(grid.width):
            # only perform gravity and brownian on sand
            if grid.get(x, y) == 's':
                # first apply gravity
...
def make_gui(top, width, height):
    ...
    scale = tkinter.Scale(top, from_=0, to=100,
                          orient=tkinter.HORIZONTAL, variable=brownian_val)
