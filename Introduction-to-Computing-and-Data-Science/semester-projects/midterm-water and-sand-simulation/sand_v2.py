...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    # 1. Boundary Rule: Ensure the destination is within bounds
    if x_to < 0 or x_to >= grid.width or y_to < 0 or y_to >= grid.height:
        return False

    # 2. Empty Rule: The destination should be empty
    if grid.get(x_to, y_to) is not None:
        return False

    # 3. Corner Rule: Applies to down-left and down-right moves
    if y_to + 1 < grid.height:
        # down-left or down-right
        if (x_from - x_to == 1 or x_to - x_from == 1) and (
                y_from - y_to == 1 or y_to - y_from == 1):
