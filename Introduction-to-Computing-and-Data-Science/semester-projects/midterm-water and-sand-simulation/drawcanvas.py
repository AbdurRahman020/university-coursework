...
class DrawCanvas(object):
    ...
    def draw_rect(self, x, y, width, height, color='black'):
        ...
        self.canvas.create_rectangle(
            x, y, x + width - 1, y + height - 1, outline=color)
        if self.auto_update:
            self.canvas.update()
    ...
    def fill_rect(self, x, y, width, height, color='black'):
        ...
        if type(color) is tuple:
            color = DrawCanvas.color_name(color)
        self.canvas.create_rectangle(
            x, y, x + width - 1, y + height - 1, outline=color, fill=color)
    ...
    def draw_oval(self, x, y, width, height, color='black'):
        ...
        if type(color) is tuple:
            color = DrawCanvas.color_name(color)
        self.canvas.create_oval(x, y, x + width - 1,
                                y + height - 1, outline=color)
        if self.auto_update:
            self.canvas.update()
    ...
    def fill_oval(self, x, y, width, height, color='black'):
        ...
        if type(color) is tuple:
            color = DrawCanvas.color_name(color)
        self.canvas.create_oval(x, y, x + width - 1,
                                y + height - 1, outline=color, fill=color)
        if self.auto_update:
            self.canvas.update()
    ...
    def draw_string(self, x, y, text, color='black'):
        ...
        self.canvas.create_text(
            x, y, text=text, anchor=tkinter.NW, fill=color, font=('Courier', 24))
        if self.auto_update:
            self.canvas.update()
...
def test_canvas(width, height):
    ...
    canvas.fill_oval(0, 0, width, height, color=(
        100, 100, 200))  # rgb tuple form
